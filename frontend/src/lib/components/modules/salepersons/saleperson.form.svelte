<script lang="ts">
    import type { Salesperson, SalespersonStatus } from '$lib/types/salesperson.types';
    ...
    let { salesperson = null, onSubmit }: Props = $props();

    const isEditMode = $derived(!!salesperson?._id);
    ...
    function validate(): boolean {
        ...
        if (!isEditMode) {
            ...
            // Optional password update during edit mode
...
<div class="rounded-xl border bg-card p-5 w-180 shadow-sm">
